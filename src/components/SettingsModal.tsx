...
const SettingsModal = ({ isOpen, onClose }: SettingsModalProps) => {
  ...
  const handleThemeChange = (theme: Theme) => {
    ...
    utools.dbStorage.setItem('theme', JSON.stringify(theme))
    applyTheme(theme)
  }
  ...
  const handleExport = () => {
    const notes = JSON.parse(utools.dbStorage.getItem('poemNotes') || '[]')
      ...
      if (file) {
        ...
        reader.onload = (event) => {
          try {
            ...
            utools.dbStorage.setItem('poemNotes', JSON.stringify(notes))
            alert('笔记导入成功！')
          } catch (error) {
            alert('导入失败，请检查文件格式！')
          }
